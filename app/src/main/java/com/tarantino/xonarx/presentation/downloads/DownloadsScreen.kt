...
class DownloadsViewModel @Inject constructor(
    ...
    val downloads: StateFlow<List<DownloadItem>> = identityManager.activeIdentity
        .flatMapLatest { identity ->
            if (identity == null) flowOf(emptyList()) else downloadRepository.observeDownloads(identity.id)
        }
    ...
    Scaffold(
        ...
        if (items.isEmpty()) {
            ...
            LazyColumn(
                ...
                items(items) { item ->
                    ListItem(
                        headlineContent = { Text(item.fileName, maxLines = 1) },
                        supportingContent = { Text("${item.status.name} - ${item.progress}%", maxLines = 1) },
                        trailingContent = {
                            IconButton(onClick = { viewModel.deleteDownload(item) }) {

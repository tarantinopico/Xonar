...
class DownloadRepositoryImpl @Inject constructor(
    ...
    override fun observeAllDownloads(): Flow<List<DownloadItem>> = dao.observeAllDownloads().map { list -> list.map { it.toDomainModel() } }
    override suspend fun getDownloadById(id: String): DownloadItem? = dao.getDownloadById(id)?.toDomainModel()
    override suspend fun addDownload(item: DownloadItem) = dao.insertDownload(DownloadItemEntity.fromDomainModel(item))
    override suspend fun updateDownload(item: DownloadItem) = dao.updateDownload(DownloadItemEntity.fromDomainModel(item))
